...
if(isset($_SESSION['numreloj'])){
		...
		$tabla = '';
		$query2 = "SELECT NumeroReloj FROM ConsultasEmp WHERE NumeroReloj = ?";
...
if(isset($_POST['guardar'])){
    ...
    //insertar consulta del paciente (empleado)
    if(!$errores){
        $insert = "INSERT INTO ConsultasEmp (NumeroReloj, Motivo, Diagnostico, Tratamiento, ExploracionFisica, Comentarios, Descripcion, Responsable, Fecha, Empresa, Planta, Enfermedades) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, GETDATE(), ?, ?, ?)";
        $param3 = array($numreloj, $motivo, $diagnostico, $tratamiento, $exploracion, 
        $comentarios, $descripcion, $responsable, $empresa, $planta, $enfermedades);
